...
var _DensitySnapshot, DensitySnapshot, MIN_CAUSAL_SPAN, MIN_PEER_CARDINALITY;
var init_snapshot = __esm({
  "src/snapshot.ts"() {
    ...
    DensitySnapshot = _DensitySnapshot;
    MIN_CAUSAL_SPAN = 1;
...
async function interpolate(snapshots) {
  ...
  if (snapshots.length === 1) {
    const s = snapshots[0];
    return s !== void 0 ? s : DensitySnapshot.ZERO;
  }
  let totalWeight = 0;
  let weightedRho = 0;
  let maxCount = 0;
...
export { CausalDensityField, DensitySnapshot, HyperLogLog, fnv32, gradient, interpolate, normalize, rankByDensity, selectDensest };
